 ...
 * 08-06-2023      1.0                 DucTM           First Implement
 */
package filter;
...
public class AccessFilter implements Filter {
    ...
    /**
     *
     * @param request_ The servlet request we are processing
     * @param response_ The servlet response we are creating
    ...
    public void doFilter(ServletRequest request_, ServletResponse response_,
        ...
        // include requests.
        RequestWrapper request = new RequestWrapper((HttpServletRequest) request_);
        ResponseWrapper response = new ResponseWrapper((HttpServletResponse) response_);
        ...
        //guest/not-logged-in user can access
        if (user == null && !Link.notLoggedIn.contains(url)) {
            response.sendRedirect(context + "/login");
            return;
        }
        //prevent access when user haven't entered a valid email to reset password
        if (key == null && Link.forgotPassword.contains(url)) {
            response.sendRedirect(context + "/forgotpassword");
            return;
        }
        if (user != null) {
            //every user can access
            if (Link.user.contains(url)) {
                chain.doFilter(request, response);
                return;
            }
            //every tourist can access
            if (user.getRole().equals("Tourist") && !Link.tourist.contains(url)) {
                response.sendRedirect("home");
                return;
            }
            //every travel agent can access
            if (user.getRole().equals("Travel Agent") && !Link.agent.contains(url)) {
                response.sendRedirect("home");
                return;
            }//admin can access
        ...
        if (problem != null) {
            ...
            sendProcessingError(problem, response_);
        }
    }
